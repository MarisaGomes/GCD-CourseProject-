...
#1.**We start by Downloading the file and puting the file  in the `data` folder**

if(!file.exists("./data")){dir.create("./data")}
fileUrl <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
download.file(fileUrl,destfile="./data/Dataset.zip",method="auto")

#2.**Then we Unzip the file** 

unzip(zipfile="./data/Dataset.zip",exdir="./data")

#3.**The unzipped files are in the folder`UCI HAR Dataset`. So we have to get the list of the files**

path_rf <- file.path("./data" , "UCI HAR Dataset")
files<-list.files(path_rf, recursive=TRUE)

##To read data from  the  targeted files
...
#2.**To read data from the files into the variables**
